...
class _DetailScreenState extends State<DetailScreen> {
  ...
  int _noOfTickets = 0;
  double _pricePerTicket = 0;
  double _cartTotal = 0;
  String? _ticketImageUrl;
  String? _language;
  String? _theatrePlace;

  List<Map<String, dynamic>> _cartTickets = [];
  ...
  Future<void> _loadUserDataAndCart() async {
    ...
    // Fetch cart from API
    if (_userId != null) {
      await _fetchCartFromAPI(_userId!);
    }
  }

  Future<void> _fetchCartFromAPI(String userId) async {
    try {
      setState(() {
        _isLoadingCart = true;
      });
      ...
      final response = await http.get(
        Uri.parse('http://31.97.206.144:8127/api/auth/getcart/$userId'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['cart'] != null &&
            data['cart']['tickets'] != null &&
            data['cart']['tickets'].isNotEmpty) {
          final ticket = data['cart']['tickets'][0]; // Get first ticket

          setState(() {
            _movieName = ticket['MovieName'];
            _showDate = DateTime.parse(ticket['showDate']);
            _showTime = ticket['showTime'];
            _ticketCategory = ticket['ticketCategory'];
            _selectedSeats = List<String>.from(ticket['selectedSeats']);
            _noOfTickets = ticket['noOfTickets'];
            _pricePerTicket = ticket['pricePerTicket'].toDouble();
            _ticketImageUrl = ticket['ticketImage'];
            _cartTotal = data['cart']['cartTotal'].toDouble();
            _language = ticket['language'] ?? 'N/A';
            _theatrePlace = ticket['theatrePlace'] ?? 'N/A';
      ...
      _showErrorSnackBar('Failed to load cart data');
    }
  }
  ...
  void _initiateRazorpayPayment() {
    var options = {
      ...
      'name': 'Movie Ticket App',
      'description':
          'Movie Ticket Purchase - $_movieName ($_noOfTickets tickets)',
      'send_sms_hash': true,
      'prefill': {
        'contact': "9961593179",
        'email': "melvincherian0190@example.com",
  ...
  Future<void> _bookTickets(String? paymentId) async {
    try {
      ...
      final response = await http.post(
        ...
        body: json.encode({'userId': _userId, 'transactionId': paymentId}),
      );
  ...
  Future<void> _processTicketPurchase(String? paymentId) async {
    try {
      ...
      print('Order Details:');
      print('Movie: $_movieName');
      print('Theatre: $_theatrePlace');
      print('Date: $_showDate');
      print('Time: $_showTime');
      print('Tickets: $_noOfTickets');
      print('Total: ₹$_cartTotal');
      print('Payment Method: $paymentMethodName');
      print('Payment ID: ${paymentId ?? 'COD'}');
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              SizedBox(height: 10),
              Text(
                'Your ticket for $_movieName has been confirmed.',
                textAlign: TextAlign.center,
                style: TextStyle(fontSize: 16),
              ),
              SizedBox(height: 10),
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (_showDate != null)
                          Text(
                            'Date: ${DateFormat('dd MMM yyyy').format(_showDate!)}',
                          ),
                        if (_showTime != null) Text('Time: $_showTime'),
                        if (_theatrePlace != null)
                          Text('Theatre: $_theatrePlace'),
                        SizedBox(height: 8),
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                    ...
                    child: ElevatedButton(
                      ...
                      child: _isProcessingOrder
                          ...
                          : Text(
                              _selectedPaymentMethod == null
                                  ? 'Select Payment Method'
                                  : 'Confirm Purchase - ₹${_cartTotal.toStringAsFixed(0)}',
                              style: TextStyle(
                                color: Colors.white,
                                fontWeight: FontWeight.bold,
                              ),
                                ...
                                children: [
                                    ...
                                    child: Stack(
                                      children: [
                                        ClipRRect(
                                          ...
                                          child:
                                              _ticketImageUrl != null &&
                                                  _ticketImageUrl!.isNotEmpty
                                              ? Image.network(
                                                  getTicketImageUrl(
                                                    _ticketImageUrl.toString(),
                                                  ),
                                                  fit: BoxFit.cover,
                                                  width: double.infinity,
                                                  height: double.infinity,
                                  ...
                                  Column(
                                    children: [
                                      ...
                                      _buildDetailRow(
                                        'Time & Date:',
                                        '${_showDate != null ? DateFormat('dd MMM yyyy').format(_showDate!) : ''} ${_showTime ?? ''}',
                                      ),
                                      _buildDetailRow(
                                        'Location:',
                                        _theatrePlace ?? 'N/A',
