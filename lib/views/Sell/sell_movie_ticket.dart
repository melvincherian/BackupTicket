...
class _SellMovieTicketState extends State<SellMovieTicket> {
  ...
  final Map<String, List<String>> hallNumbersByType = {
    'Screen': List.generate(9, (index) => (index + 1).toString()),
    'Audi': List.generate(9, (index) => (index + 1).toString()),
  };
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<MovieTicketProviderapi>(
        context,
        listen: false,
      ).fetchMovieNames();
    });
    ...
    try {
      ...
      if (image != null) {
        final result = await Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) =>
                ImageCropperScreen(imageFile: File(image.path), isQrCrop: true),
          ),
        );
  ...
  Future<void> _submitForm() async {
    ...
    //     const SnackBar(
    //       content: Text(
    //         'Show time has already passed. Please select a future show.',
    //       ),
    //       backgroundColor: Colors.red,
    //     ),
    //   );
    ...
    final String hallLabel = '${_selectedHallType!} ${_selectedHallNumber!}';
    ...
    try {
      ...
      final provider = Provider.of<MovieTicketProviderapi>(
        context,
        listen: false,
      );
      final userId = await SharedPrefsHelper.getUser();
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              child: ClipRRect(
                borderRadius: BorderRadius.circular(8),
                child: Image.asset('assets/sample.png', fit: BoxFit.contain),
              ),
            ),
            Padding(
                ...
                children: [
                  ...
                  if (_selectedHallType != null)
                    _buildDropdownField(
                      '${_selectedHallType} Number',
                      'Select number',
                      hallNumbersByType[_selectedHallType] ?? [], // <-- new
                      _selectedHallNumber,
                      (value) {
                        setState(() {
                          _selectedHallNumber = value;
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          ...
          onChanged: (value) {
            setState(() {
              _selectedMovieName = value;
              _selectedMovieId = movieNames
                  .firstWhere((movie) => movie.movieName == value)
                  .id;
              _selectedSeats.clear();
            });
          },
        ...
        if (_selectedQrImage != null) ...[
          ...
          Center(
            child: Container(
              ...
              child: ClipRRect(
                borderRadius: BorderRadius.circular(6),
                child: Image.file(_selectedQrImage!, fit: BoxFit.cover),
              ),
            ),
          ),
          const SizedBox(height: 8),
