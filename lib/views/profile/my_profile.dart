...
class _MyProfileState extends State<MyProfile> {
  ...
  Future<void> _loadUserProfile() async {
    ...
    try {
      ...
      print('ussssssssssssssssseeeeeeeeeeeeeer $_userId ');
  ...
  Future<void> _handleSave() async {
    ...
    try {
      ...
      File? imageToUpload = _selectedImageFile;

      if (kIsWeb && _selectedImageBytes != null) {
        await _handleSaveFallback();
        return;
      }

      // Use provider to update profile
      ...
      if (success) {
        ...
        _showSnack(provider.errorMessage ?? 'Failed to update profile');
      }
    } catch (e) {
      _showSnack('Error updating profile: $e');
    }
  ...
  /// ================= FALLBACK SAVE (for web or direct API) =================
  Future<void> _handleSaveFallback() async {
    try {
      final token = await SharedPrefsHelper.getToken();
      if (token == null) {
        _showSnack('Authentication token not found');
    ...
    return Scaffold(
      ...
      body: Consumer<ProfileProvider>(
        builder: (context, provider, child) {
          // Show loading indicator when either local loading or provider is loading
